...
| 段階 | 内容 | 主な対象 | 規模 |
|---|---|---|---|
| **A. 共通パネル枠 + 固有ツールバー** | `Panel` コンポーネント(統一ヘッダ:タイトル/折りたたみ/⋯プレースホルダ + 本体 + 固有ツールバー枠)を作り、既存ツールパネル(Files/Parts/Properties/Palette)を載せ替え。**cvox/rig/anim を Preview 右上のフローティングへ**移設。CSS 統一(P4-1 と相性良)。**配置は現状のまま** | 新規 `Panel.tsx`、各パネル、`App.tsx`、`styles.css` | M |
...
| **C. タブ + 移動/分割 + 永続化** | 葉のタブ化。⋯メニューで「移動 / 分割 / タブ化 / 並べ替え / 閉じる」= ツリー組み替え。`Layout` 全体を localStorage 保存。「レイアウトをリセット」 | `Dock`、`lib/layout.ts` | M |
...
### 各段階の完了条件(目安)
- **A**: 全ツールパネルが同一ヘッダ/折りたたみ。Preview 右上に cvox/rig/anim。配置は不変
- **B**: 初期配置が再帰 split で描画され、スプリッタでリサイズ&保存できる。#5/#6 解消
- **C**: パネルを移動・分割・タブ化でき、再読込後も配置保持。リセット可
