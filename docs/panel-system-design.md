...
### 操作 = ツリーの組み替え(実装済み, D&D で行う)
- **タブ化/移動/並べ替え** … **ヘッダー(タブ行)にドロップ**。タブの上=前/後に挿入、空きスペース=一番右へ追加。挿入ラインは tab offset から算出した境界 x に1本(`placePanelBeside`)
- **分割** … **本体(ボディ)にドロップ** → 近い縁の方向に2分割(`splitLeafWith`)
- **リサイズ** … スプリッタをドラッグ(`SplitNode.ratio` 更新, `withRatioAt`)
- **閉じる** … パネルのタブ × でツリーから外す(`closePanelAt`、空になった split は相方に畳む)。**折りたたみは廃止**
- **パネル追加** … 各葉のタブ行の **+** で、閉じた(未配置の)パネルを一覧から追加(`addPanelAt`)
