...
## P1 — タイムラインの精度・可視性 `[#2]` 系 — 🟡 一部完了(チャンク1)

チャンク1(P1-1/2/3)を 2026-06-30 に実装。スナップ単位は **0.05s 固定**(`Alt` で 1e-3 バイパス)を採用。
...
| P1-5 `[new]` | キーボード: 選択キーの `Delete`、`Space` で再生/停止、矢印で1フレームナッジ。IMEガードは既存パターン流用 | M | `App.tsx`, `Timeline.tsx` | ⬜ |
| P1-6 `[new]` | レーン背景クリックでスクラブ(/ダブルクリックでキー追加)。今は背景が死んでいる | S/M | `Timeline.tsx` | ⬜ |
