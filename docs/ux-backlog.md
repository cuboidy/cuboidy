...
## P1 — タイムラインの精度・可視性 `[#2]` 系 — 🟡 一部完了(チャンク1・2)

チャンク1(P1-1/2/3)＋チャンク2(P1-5/6)を 2026-06-30 に実装。スナップ単位は
**0.05s 固定**(`Alt` で 1e-3 バイパス)を採用。残りは P1-4(横ズーム)・P1-7(範囲外キー)。

| ID | 課題 | 規模 | 対象 | 状態 |
|---|---|---|---|---|
| P1-1 `[#2]` | キーフレームドラッグの粗いスナップ。**0.05s 固定グリッド**、`Alt` で微調整(1e-3)バイパス。1e-3 は保存グリッドとして維持 | M | `Timeline.tsx` | ✅ |
| P1-2 `[#2]` | ルーラーに目盛り＋時刻ラベル、レーンに薄いグリッド線(スナップ先を可視化)。間隔は nice step で自動 | M | `Timeline.tsx`, `styles.css` | ✅ |
| P1-3 `[new]` | 新規クリップで最初のキーを追加できない(全行が畳まれ `+` が隠れる)。空クリップは先頭パートを自動展開 | S | `Timeline.tsx` | ✅ |
| P1-4 `[new]` | 横ズーム(pixels-per-second)＋横スクロール。密なキーが重なってクリック不能になる問題の根治 | M/L | `styles.css`, `Timeline.tsx` | ⬜ |
| P1-5 `[new]` | キーボード: 選択キーの `Delete`/`Backspace`、`Space` で再生/停止、`←/→` で 0.05s ナッジ(`Alt` で 1e-3)。IME・テキストフィールド・ボタンフォーカスをガード | M | `AnimationView.tsx` | ✅ |
| P1-6 `[new]` | レーン背景クリック/ドラッグでスクラブ(マーカー上は従来通り選択/移動)。ダブルクリック追加は見送り | S/M | `Timeline.tsx`, `styles.css` | ✅ |
| P1-7 `[new]` | duration 短縮で範囲外になったキーが右端に重なって不可視。扇状に広げる or バッジtooltipで個別選択 | S | `Timeline.tsx` | ⬜ |

---

## P2 — レイアウト / 情報設計 `[#5/#6]` → ドックパネルシステムへ拡張

> **方針変更(2026-06-30):** 固定レイアウトの組み替えではなく、**ドック可能パネル
> システム**(共通規格・移動・タブ・折りたたみ・永続化)に作り替える。設計とロード
> マップは **`docs/panel-system-design.md`** を参照。下表は「初期配置」として活かす。
> P2-4(リサイズ)は Phase B に内包、P2-1/2/3 は初期配置で実現。

目標IA(= ドックの初期配置): **左ゾーン**=Files → Parts → Properties を縦スタック
(選択→編集が近い, #5)、**右ゾーン**=Palette 単独(rig から分離, #6)、rig/anim では
右を畳んで中央拡幅。
