...
Notes captured while hand-building characters. Seed for a future
"author a Cuboidy model" skill. The golden rule: **write the geometry file by
hand and let the CLIs give you feedback** — do not generate it from a script
(that defeats the point of a readable format and hides what hand-authoring can
do).
...
- Default pivot = `[W/2, 0, D/2]` (bottom-center) — omit `pivot` entirely to get
  it. That also avoids the H02 fractional-pivot hint on odd sizes.
