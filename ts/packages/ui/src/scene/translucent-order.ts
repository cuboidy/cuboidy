...
// The originals are snapshotted rather than rebuilt from a base vertex, so
// this stays correct if that triangulation ever changes.
const INDICES_PER_QUAD = 6;

// three.js calls `object.onBeforeRender(renderer, scene, camera, …)`; only
// the camera is needed here, and taking it from the call rather than from a
// closure is what lets the same sorter serve r3f and the plain-three
// thumbnail renderer, which never share a camera object.
...
const NOTHING_TO_SORT: BeforeRender = () => {};
...
export function makeTranslucentSorter(
  ...
  return (_renderer, _scene, camera) => {
    const obj = object();
    if (obj === null) return;
    modelView.multiplyMatrices(camera.matrixWorldInverse, obj.matrixWorld);
    // A material array makes three.js call this once per group, and an
    // editor camera is stationary most frames. Both are answered by
    // re-sorting only when the view actually moved.
